...
```powershell
. ".\AuthN_AuthZ.ps1"  # Make sure this sets the $Token variable
...
Write-Host "Script started at: $StartTime UTC"

# Import the CSV file with a column 'Id'
$ServicePrincipals = Import-Csv -Path ".\Output\06\ServicePrincipals_Report.csv"
...
for ($i = 0; $i -lt $Batches; $i++) {
    Write-Host "Processing batch $($i + 1) of $Batches..."

    $StartIndex = $i * $BatchSize
    $EndIndex = [Math]::Min($StartIndex + $BatchSize - 1, $Total - 1)
    $CurrentBatch = $ServicePrincipals[$StartIndex..$EndIndex]

    foreach ($sp in $CurrentBatch) {
        $Endpoint = "/servicePrincipals/$($sp.Id)/appRoleAssignedTo"
        $Uri = "$BaseApi/$ApiVersion$Endpoint"
        ...
        try {
            ...
            foreach ($assignment in $response.value) {
                ...
                if (-not $PrincipalName -or $PrincipalName -eq "") {
                    try {
                        $lookupUri = "$BaseApi/$ApiVersion/directoryObjects/$($assignment.principalId)"
                        $lookup = Invoke-RestMethod -Uri $lookupUri -Headers $Headers -Method GET
                        $PrincipalName = $lookup.displayName
                        $odataType = $lookup.'@odata.type'
                        ...
                        switch ($odataType) {
                            ...
                            default { $PrincipalType = $odataType }
                        }
                    }
                    catch {
                        $PrincipalName = "<lookup failed>"
                        $PrincipalType = "<lookup failed>"
                    }
                }
                else {
                    # Still attempt lookup to determine type if name is already present
                    try {
                        $lookupUri = "$BaseApi/$ApiVersion/directoryObjects/$($assignment.principalId)"
                        $lookup = Invoke-RestMethod -Uri $lookupUri -Headers $Headers -Method GET
                        $odataType = $lookup.'@odata.type'
            ...
            Write-Warning "Failed to fetch role assignments for SP ID: $($sp.Id) - $($_.Exception.Message)"
        }
    }
}

# Export results to CSV
$Results | Export-Csv -Path ".\Output\06\AppRoleAssignments_Report.csv" -NoTypeInformation -Encoding UTF8
Write-Host "Export completed to .\Output\06\AppRoleAssignments_Report.csv"

# Track end time and duration
$EndTime = [datetime]::UtcNow
$Duration = $EndTime - $StartTime
Write-Host "Script ended at:   $EndTime UTC"
